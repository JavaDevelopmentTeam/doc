CREATE DATABASE IF NOT EXISTS `springcloud_sell` DEFAULT CHARSET utf8 COLLATE utf8_general_ci;
...
create table `order_master` (
  ...
  `order_status` tinyint(3) not null default '0' comment '订单状态,默认为新下订单',
  `pay_status` tinyint(3) not null default '0' comment '支付状态,默认为未支付',
  `create_time` timestamp not null comment '创建时间',
  `update_time` timestamp not null comment '修改时间',
  primary key (`order_id`),
  key `idx_buyer_id` (`buyer_openid`)
)default charset = utf8;
